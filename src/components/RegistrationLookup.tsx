...
export function RegistrationLookup({ variant = "default" }: { variant?: "default" | "sidebar" }) {
  ...
  const lookupQuery = useRegistrationLookup(searchReference, searchReference.length > 0);
  ...
  function handleReceiptUpload(e: FormEvent) {
    ...
    if (!receiptFile || !lookupQuery.data) return;
    ...
    requestConfirm({
      ...
      action: async () => {
        ...
        try {
          ...
          await submitReceipt(registration.referenceNumber, file);

          setReceiptFile(null);

          await queryClient.invalidateQueries({

            queryKey: queryKeys.registrations.lookup(registration.referenceNumber),
  ...
  const canUploadReceipt =
    ...
    (lookupQuery.data.paymentStatus === "pending" ||

      lookupQuery.data.paymentStatus === "receipt_issue" ||

      lookupQuery.data.paymentStatus === "rejected");
  ...
  return (
    ...
    >
      ...
      <p className={`text-sm mb-4 ${isSidebar ? "registration-sidebar-muted" : "text-muted"}`}>
        ...
        Enter your reference number to verify your registration and upload payment proof.
      ...
      <form onSubmit={handleLookup} className="d-flex gap-2">
        ...
        <input
          ...
          placeholder="PNA-2026-XXXXX"

          className={`flex-grow-1 uppercase ${isSidebar ? "registration-sidebar-input" : "input-dark"}`}
        ...
        <button
          ...
          className={`btn-primary shrink-0 ${isSidebar ? "registration-sidebar-btn" : "!py-2.5 !px-4 !text-xs"}`}
      ...
      {lookupQuery.isSuccess && lookupQuery.data && (
        ...
        >
          ...
          {canUploadReceipt && (
            ...
            <form onSubmit={handleReceiptUpload} className="mt-4 pt-3 border-top border-white border-opacity-10">

              <p className={`small mb-2 ${isSidebar ? "registration-sidebar-text" : "text-ink"}`}>

                Upload proof of payment ({PAYMENT_STATUS_LABELS[lookupQuery.data.paymentStatus]})
          ...
          {uploadError && (
            ...
            <p className={`mt-3 mb-0 small ${isSidebar ? "registration-sidebar-error" : "text-red-400"}`}>
